...
import BrowserStorageService from "@/services/localStroage/BrowserStorageService.js";
...
function Order() {
  ...
  const handleSignInClose = () => {
    const user = JSON.parse(BrowserStorageService.get('User'));
    if (user) {
  ...
  const removeItem = (itemId) => {
    ...
    const updatedOrder = { ...order, items: updatedItems, totalAmount: updatedTotalAmount };
  ...
  return (
    <>
      {alert && <div className="absolute top-4"><Alert userName={alert} close={handleAlertClose} /></div>}
      <div className="m-auto px-4 py-16 bg-stone">
        ...
        <div className="container px-5 mx-auto flex sm:flex-nowrap flex-wrap">
          <div className="lg:w-2/3 md:w-1/2 sm:mr-10 mt-2 flex items-end justify-start relative">
            <Image alt="restaurant" src="/Restaurant.png" width={400}
              height={400} className="object-cover w-[300px] h-[300px] sm:w-[660px] sm:h-[500px] object-center block" />
          </div>
